...
import { getStudentsByParentEmail } from "../../services/studentService";

// This value is used when auth is not connected yet
// Change it to the real parent email for testing
const PARENT_EMAIL = "anna@gmail.com";
...
export default function Dashboard() {
  ...
  // students is loaded from Firestore for this parent email
  const [students, setStudents] = useState([]);
  const [loadingStudents, setLoadingStudents] = useState(true);
  const [studentsError, setStudentsError] = useState("");

  const parentEmail = useMemo(() => {
    // This is a simple setup for now
    // Later can replace it with the logged in user email
    return PARENT_EMAIL;
  }, []);

  useEffect(() => {
    // Load students that match this parent email
    const loadStudents = async () => {
      try {
        setLoadingStudents(true);
        setStudentsError("");
        const data = await getStudentsByParentEmail(parentEmail);
        setStudents(data);
      } catch (error) {
  ...
  const renderEnrollment = () => (
    <div className="pagePad">
      <div className="eventDetailBlock">
        <div className="eventDetailTitle">Student Overview</div>
  ...
  const renderManage = () => (
    <div className="pagePad">
      <div className="eventDetailBlock">
        <div className="eventDetailTitle">Student Info</div>
        <div className="eventDetailDesc">This list is loaded from Firestore</div>
  ...
  const renderContact = () => (
    <div className="pagePad">
      <div className="eventDetailBlock">
        <div className="eventDetailTitle">Contact School</div>
        ...
        <div className="modalForm" style={{ marginTop: 8 }}>
          ...
          <label className="modalLabel">
            Email
            <input className="modalInput" placeholder={parentEmail} />
          </label>
